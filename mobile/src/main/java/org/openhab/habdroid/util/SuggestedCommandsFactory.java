...
public class SuggestedCommandsFactory {
    ...
    public SuggestedCommands fill(@Nullable Widget widget) {
        ...
        if (widget.hasMappingsOrItemOptions()) {
            for (LabeledValue mapping : widget.getMappingsOrItemOptions()) {
                add(suggestedCommands, mapping.getValue(), mapping.getLabel());
            }
        }
        ...
        if (widget.getType() == Widget.Type.Setpoint || widget.getType() == Widget.Type.Slider) {
            if (widget.getState() != null && widget.getState().getAsNumber() != null) {
                ...
                add(suggestedCommands, ParsedState.NumberState.Companion.withValue(state,
                        widget.getMinValue()).toString());
                add(suggestedCommands, ParsedState.NumberState.Companion.withValue(state,
                        widget.getMaxValue()).toString());
        ...
        return fill(widget.getItem(), suggestedCommands);
    }
    ...
    private SuggestedCommands fill(@Nullable Item item, SuggestedCommands suggestedCommands) {
        ...
        if (item.isOfTypeOrGroupType(Item.Type.Color)) {
            ...
            if (item.getState() != null && item.getState().getAsNumber() != null) {
                add(suggestedCommands, item.getState().getAsNumber().toString());
            }
